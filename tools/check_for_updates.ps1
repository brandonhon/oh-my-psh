...
function Add-CurrentTime {
  Get-Date -Format d | Set-Content "$Env:USERPROFILE/.oh-my-psh-update"
}

if ($AutoUpdate -eq $true) {
  # Compare current date to last update date
  if (Test-Path "$Env:USERPROFILE/.oh-my-psh-update") {
    $oldDate = Get-Content "$Env:USERPROFILE/.oh-my-psh-update" | Get-Date -Format d
  ...
  if ($update){
    ...
    if ($confirmation -ieq "y"){
        ...
        & "$Env:USERPROFILE\.oh-my-psh\update.ps1"
        Add-CurrentTime
        $update = 0
    } else {
        "Update canceled"
